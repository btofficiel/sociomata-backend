...
const dispatchTweets = async (id, post_id, db, sql, env) => {
    ...
    if(credentials) {
        ...
        const mediaList = await db.many(sql.fetch_media, [post_id]); 
...
const dispatchTweetsNow = async (id, thread, db, sql, env) => {
    ...
    if(credentials) {
        ...
        let mediaFlattened = thread.reduce(flattenMediaArrays, []);
        
        await sendTweets(thread, mediaFlattened, twitterClient, true);
