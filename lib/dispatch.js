...
const dispatchTweets = async (account, post_id, db, sql, env) => {
    const { plug_id, social_account } = await db.one(sql.fetch_post, [post_id, account]);
    const credentials = await db.oneOrNone(sql.fetch_auth, [social_account, env.TWITTER_DB_KEY, env.DB_ENCRYPTION_ALGO, account]);
    if(credentials) {
...
const dispatchTweetsNow = async (account, thread, media, plug_id, social_account, db, sql, env) => {
    const credentials = await db.oneOrNone(sql.fetch_auth, [social_account, env.TWITTER_DB_KEY, env.DB_ENCRYPTION_ALGO, account]);
    if(credentials) {
        const twitterClient = new TwitterClient({
            apiKey: env.TWITTER_APIKEY,
