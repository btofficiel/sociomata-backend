...
const addSubscription = async (account, plan, config, db, sql) => {
    const {
        customer_id,
    ...
    await db.none(sql, [account, customer_id, current_subscription, current_period_end, plan]);
...
const fetchPlans = async (db, sql) => {
    let plans = await db.many(sql, []);
    return api.createSuccessResponse(200, { plans });
};
