    ...
    return new Promise((resolve, reject)=>{
        ...
        for(let post of postList) {
            if(post.timestamp >= currentDayStart && post.timestamp <= currentDayEnd) {
                if(posts[currentDayStart]) {
                    posts[currentDayStart].push(post);
                } 
                else {
                    posts[currentDayStart] = [post];
                }
            }
            else {
...
const fetchPosts = async (id, db, sql) => {
    ...
    const timestampTodayUTC = Math.floor(getUnixTime(Date.now())/86400)*86400;
    const timestampToday = timestampTodayUTC-(offset*60);
    const timestamp3DaysAhead = timestampToday+(86400*3)-60;

    const postsList = await db.manyOrNone(sql.posts, [id, timestampToday, timestamp3DaysAhead]);

    const posts = await getPostsAsDict(timestampToday, postsList);
