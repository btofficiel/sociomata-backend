...
const createPost = async (payload, id, db, sql) => {
    ...
    if(twitterConnection) {
        ...
        let mediaFlattened = tweets.reduce(flattenMediaArrays, []);

        /* istanbul ignore next */
        let mediaKeys = await Promise.all(mediaFlattened.map(utils.uploadMedia));
        ...
        await db.tx('createPostTx', async t => {
            ...
            return addTwitterMedia(mediaKeys, post.id, t, sql.add_media);
        })
...
const editPost = async (payload, post_id, id, db, sql) => {
    ...
    let unchangedKeys = mediaFlattened
        .filter(i=>!i.newly_added)
        .map(m=>`twitter_images/${m.url.split('twitter_images/')[1]}`);
    ...
    await db.tx('editPostTx', async t => { 
        ...
        let removedKeys = await t.manyOrNone(sql.fetch_removed_media, [post_id, unchangedKeys]);
