...
const createPost = async (payload, id, account, db, sql) => {
    ...
    let connection = await db.oneOrNone(sql.twitter, [social_account, account]);
    if(connection) {
        ...
        await db.tx('createPostTx', async t => {
            const post = await t.one(sql.create_post,  [description, id, 1, category_id, plug_id, social_account]); 
...
const editPost = async (payload, post_id, id, account, db, sql) => {
    ...
    await db.tx('editPostTx', async t => { 
        const post = await t.one(sql.edit_post,  [description, post_id, category_id, social_account, plug_id]); 
