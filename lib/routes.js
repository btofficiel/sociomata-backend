...
const registerRoutes = (server, client) => {
    ...
    server.route({
        ...
        handler: async (request, h) => {
            try {
                let result = await fetchSocialAccounts(request.app._account, client, twitterAuth.fetch_socials);

                return h.response(result).code(result.statusCode);
            }
            catch(e) {
                const errorString = String(e.stack);
                logger.error("fetchSocials error", { 
                    timestamp: new Date(), 
                    path: request.path,
                    error: errorString,
        ...
        handler: async (request, h) => {
            try {
                let sql = {
                    ...
                    check_if_admin: user.check_if_admin
                }

                let result = await fetchMembers(request.app._account, request.app._id, client, sql);

                return h.response(result).code(result.statusCode);
            }
            catch(e) {
                const errorString = String(e.stack);
                logger.error("fetchMembers error", { 
                    timestamp: new Date(), 
                    path: request.path,
                    error: errorString,
                    userid: request.app._id,
    ...
    server.route({
        ...
        handler: async (request, h) => {
            try {
                let sql = {
                    ...
                    max_user_reached: user.check_if_max_users_reached,
                    fetch_invite: user.fetch_invite
                };

                let result = await joinTeam(request.payload, client, bcrypt, sql);
        ...
        options: {
            auth: false,
            validate: {
                payload: validators.users["POST/join"]
            }
        }
    });
    //Invite routes 
    server.route({
        ...
        handler: async (request, h) =>{
            try {
                let sql = {
                    ...
                    check_if_admin: user.check_if_admin,
                    max_user_reached: user.check_if_max_users_reached,
                    add_invitee: user.add_invitee 
                }
                let result = await inviteMember(request.payload, request.app._account, request.app._id,  client, sql);
        ...
        options: {
            auth: 'jwt',
            validate: {
                payload: validators.users["POST/invite"] 
            }
        }
    });
    ...
    server.route({
        ...
        handler: async (request, h) => {
            try {
                let sql = payments.fetch_plans;

                let result = await fetchPlans(client, sql);

                return h.response(result).code(result.statusCode);
            }
            catch(e) {
                const errorString = String(e.stack);
                logger.error("fetchPlans error", { 
                    timestamp: new Date(), 
                    path: request.path,
                    error: errorString,
                    userid: request.app._id,
            ...
            try {
                ...
                switch (eventType) {
                    case 'checkout.session.completed':
                        ...
                        planId = Number(subscription.metadata.plan_id)

                        await client.tx('checkoutTx', async t=> {
                            result = await addSubscription(account_id, planId, config, t, payments.add_subscription); 
                            return updateLimits(account_id, planId, t, user.update_limits);
                        });

                        return h.response(result).code(result.statusCode);
                    case 'invoice.paid':
                        subscription = await stripe.subscriptions.retrieve(data.object.subscription);
                        intent = (data.object.payment_intent ? (await stripe.paymentIntents.retrieve(data.object.payment_intent)) : null);
                        paymentMethod = (intent ? (await stripe.paymentMethods.retrieve(intent.payment_method)) : null);


                        config = {
                            invoice: data.object.id,
                            currency: (data.object.currency? data.object.currency: "Forever Free Coupon"),
                            amount: (data.object.total===0 ? 0 :Math.floor(data.object.total/100)),
                            card_brand: (paymentMethod ? getCardID(paymentMethod.card.brand) : null),
        ...
        handler: async (request, h) => {
            try {
                ...
                if(isValid) {
                    ...
                    let plan = await client.one(payments.fetch_plan_by_price, [priceId]);
    ...
    server.route({
        ...
        handler: async (request, h) => {
            try {
                let sql = {
                    profile: profiles.fetch_profile,
                    twitter: twitterAuth.fetch_socials,
                    check_if_admin: user.check_if_admin,
    ...
    server.route({
        ...
        handler: async (request, h) => {
            try {
                let hasEmptyUserSlots = await client.oneOrNone(user.check_if_max_accounts_reached, [request.app._account]);
                if(!hasEmptyUserSlots) {
                    return api.createFailedResponse(409, "Your team has already reached the maximum social accounts limit");
                }
                ...
                try {
                    ...
                    const userClient = new TwitterApi({
                      ...
                      // or if you want a app-only client (see below)
                            ...
                            accessSecret: twitter_secret,
                    });

                    let {data} = await userClient.v2.me();

                    const rows = await storeTokens(twitterAuth.upsert_auth, [
                        twitter_token, 
                        twitter_secret, 
        ...
        options: {
            ...
            handler: async (request, h) => {
                try {
                    ...
                    data.append('client_id', 1048992929327084);
                    data.append('client_secret', '60d438142b7ff5db7dae5db89f45fc2d');
                    data.append('code', request.query.code);
                    data.append('grant_type', 'authorization_code');
                    data.append('redirect_uri', `https://${process.env.NGROK}/api/connect/instagram`);

                    let token = await fetch('https://api.instagram.com/oauth/access_token',{
                      method: 'POST',
                      body: data 
                    });
                    ...
                    return h.response('Instagram logged');
                }
                catch(e) {
                    const errorString = String(e.stack);
                    logger.error("Connect Instagram error", { 
                        timestamp: new Date(), 
                        path: request.path,
                        error: errorString,
                        userid: request.app._id,
    ...
    server.route({
        ...
        handler: async (request, h) => {
            try{
                ...
                let params = {
                    client_id: '291107859775835',
                    redirect_uri: `https://${process.env.NGROK}/api/connect/facebook`,
                    scope: permissions.join(), 
                    state: request.query.token
                };

                const url = 'https://www.facebook.com/v13.0/dialog/oauth?'+new URLSearchParams(params);
                return h.redirect(url);
            }
            catch(e) {
                const errorString = String(e.stack);
                logger.error("Initiate Facebook error", { 
                    timestamp: new Date(), 
                    path: request.path,
                    error: errorString,
                    userid: request.app._id,
    ...
    server.route({
        ...
        handler: async (request, h) => {
            try {
                    ...
                    let userToken = await getFacebookAccessToken(request.query.code);
                    let pageTokens = await getFacebookPageTokens(userToken.access_token);

                    return h.redirect('/app/settings');
                }
                catch(e) {
                    const errorString = String(e.stack);
                    logger.error("Connect Facebook error", { 
                        timestamp: new Date(), 
                        path: request.path,
                        error: errorString,
                        userid: request.app._id,
    ...
    server.route({
        method: 'DELETE',
        path: '/connect/twitter/{social_id}',
        handler: async (request, h) => {
            try {
                const result = await deleteTwitterTokens(request.app._id, request.params.social_id, client, twitterAuth.disconnect);
                return h.response(result).code(result.statusCode);
            }
            catch(e) {
                const errorString = String(e.stack);
    ...
    server.route({
        ...
        handler: async (request, h) => {
            try {
                ...
                const result = await dispatchTweetsNow(
                    request.app._account, 
                    request.payload.tweets, 
                    request.payload.media, 
                    request.payload.plug_id, 
                    request.payload.social_account,
                    client, 
                    sql, 
                    process.env
                );
