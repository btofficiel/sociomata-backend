...
const registerRoutes = (server, client) => {
        ...
        options: {
            ...
            handler: async (request, h) => {
                try {
                    ...
                    if(!token) {
                        return `Authentication failed due to missing access token`;
                    }
                    ...
                    if(!isValid) {
                        return `Authentication failed due to invalid access token`;
                    }

                    const rows = await client.one(twiiterAuth.upsert_auth, [
                        twitter_token, 
                        twitter_secret, 
                    ...
                    return h.redirect('/api/notfound');
                }
                catch(e) {
                    const errorString = String(e.stack);
    ...
    server.route({
        ...
        handler: async (request, h) => {
            try {
                const result = await editPost(request.payload, request.params.post_id, request.app._id, client, post.edit_post);
                return h.response(result).code(result.statusCode);
            }
            catch(e) {
                const errorString = String(e.stack);
